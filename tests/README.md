...
node tests/tackles_check.js               # ~3s  per-player tackles and TFL
...
| `tackles_check.js` | Per-player tackles, added Aug 2026. The app always captured "tackled by" and threw it away. Checks a sack counts as a tackle AND a sack (how stat sheets treat it), that a gain never counts as a TFL, and that an unnamed tackler credits NOBODY — not even a TEAM bucket, unlike sacks, because "team tackles" is meaningless when every play ends in one. Also asserts the **deliberate disagreement**: team TFL must EXCEED per-player TFL when a loss had no tackler named. Team TFL is a ceiling (counts from the play log), per-player is a floor (needs a name). If they ever match exactly, someone has probably "fixed" the team count and lost the ceiling. |
